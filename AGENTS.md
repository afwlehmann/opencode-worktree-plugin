...
- **Tests**: co-located `*.test.ts` files, run with `nix develop -c npm test`. Integration tests spawn `opencode` from PATH — inside `nix develop -c` the devshell's pinned, unwrapped opencode (v1.18.25 via the `opencode` flake input) shadows any wrapper on PATH; a wrapper that injects global config would co-load other plugins and break the pedantic test (self-validating: no ask is published → no `auto-approved` log → loud failure). Credentials come from a git-ignored `.env` (`OPENAI_API_KEY` or `OPENAI_API_KEY_FILE`; a directly set key wins over the file) plus `OPENAI_MODEL`/`OPENAI_URL`. The pedantic transparent approval is integration-tested against `opencode serve` + the v1 HTTP API, because `opencode run` resolves permission asks itself (`--auto` approves, otherwise auto-rejects) and can never validate the plugin's reply.
