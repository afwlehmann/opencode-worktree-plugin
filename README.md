...
- **TUI status bar** — tracks the worktrees the agent is working on via the session's `worktree_*` tool calls and shows the latest worktree name (e.g. `config-feat`; `config-fix (3)` when several are concurrently active, with the total count). Clicking the label opens a dialog listing all active worktrees with their absolute paths; selecting one copies the path to the clipboard (`pbcopy`/`wl-copy`/`xclip`/`xsel`) and confirms with a toast. The branch name is only shown when the live branch diverges from the worktree name (e.g. `config-feat:main`); outside a tracked worktree it falls back to the session directory's `<directory>:<branch>` (e.g. `config:main`), with a session-status tag while the agent is working (`[busy]`, `[retry 2]` during provider retries) and a `nix` badge when `preferNixDevelop` is active
