...
- **TUI status bar** — tracks the worktrees the agent is working on via the session's `worktree_*` tool calls and shows the worktree name(s) (e.g. `config-feat`, joined with `+` when several are concurrently active) in the `app_bottom` slot; the branch name is omitted while it matches the worktree name and only shown when it diverges (e.g. `config-feat:main`). Outside a tracked worktree it falls back to the session directory's `<directory>:<branch>` (e.g. `config:main`), with a session-status tag while the agent is working (`[busy]`, `[retry 2]` during provider retries) and a `nix` badge when `preferNixDevelop` is active
