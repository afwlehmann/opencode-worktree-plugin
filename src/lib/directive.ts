...
const permissionBullet = (mode: PermissionMode): string =>
  mode === "pedantic"
    ? [
        "- external_directory permissions — the plugin auto-approves",
        "  external_directory access for its active worktrees (paths created by",
        "  `worktree_create` that have not been merged/removed yet), without",
        "  prompting the user. A raw `git worktree add` produces a directory",
        "  that is NOT an active plugin worktree, so its reads and edits fall",
        "  back to per-path approval prompts.",
      ].join("\n")
    : [
...
export const worktreeDirective = (mode: PermissionMode): string =>
  [
    ...
    "`git worktree add`, `git worktree remove`, `git merge <branch>`",
    ...
    "  worktree.",
    "- merge safety — `worktree_merge` applies the configured merge strategy",
    "  (fast-forward-only by default, or the repository's `merge.ff` when the",
    "  plugin is configured with mergeStrategy=repo-config), never checks out",
    "  another branch in the main working copy, and rolls back conflicted merges",
    "  so no half-merged state is left behind.",
