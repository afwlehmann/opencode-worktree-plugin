...
export const removeWorktreeTool = (deps: RemoveWorktreeDeps) =>
  tool({
    ...
    async execute(args, context) {
      if (!isValidWorktreeName(args.repo_short) || !isValidWorktreeName(args.branch)) {
        const name = isValidWorktreeName(args.repo_short) ? args.branch : args.repo_short
        return formatError({ kind: "invalid-name", name })
      }

      const worktreePath = await resolveWorktreePath(deps.exists, args.repo_short, args.branch)
